...
export const FooterPlayer = () => {
  ...
  // ==================== BẮT ĐẦU LOGIC LIKE MỚI (TỰ CHỮA LÀNH) ====================
  // Thêm một state cục bộ để ép UI đồng bộ khi Context bị trôi
  const [localLikeSync, setLocalLikeSync] = useState<boolean | null>(null);

  // Reset state cục bộ mỗi khi bài hát thay đổi
  useEffect(() => {
    setLocalLikeSync(null);
  }, [currentSong?.id]);

  // 4. DERIVED STATE: Lấy trạng thái like (Ưu tiên local state nếu có biến động)
  const currentlyLiked = useMemo(() => {
    if (localLikeSync !== null) return localLikeSync;
    return currentSong ? isLiked(currentSong.id) : false;
  }, [currentSong?.id, isLiked, localLikeSync]);
  ...
  const handleToggleLike = async () => {
    ...
    try {
      ...
      // Nếu API gọi thành công mượt mà, ta tự động đảo state cục bộ để UI phản hồi ngay lập tức
      setLocalLikeSync(!currentlyLiked);
      
    } catch (error: any) {
      // BẮT LỖI TỪ BACKEND ĐỂ CHỮA LÀNH UI
      const errorMsg = error?.response?.data?.message || error?.message || "";
      console.error("❌ Lỗi khi like/unlike bài hát:", errorMsg);
      
      // Kịch bản 1: Front-end tưởng chưa Like, gửi lệnh Like -> Back-end chửi "Đã like rồi"
      if (errorMsg.includes('already liked') || errorMsg.includes('already')) {
        toast.info("Đã đồng bộ: Bài hát này đã nằm trong mục Yêu thích!");
        setLocalLikeSync(true); // Ép trái tim xanh lên!
      } 
      // Kịch bản 2: Front-end tưởng đã Like, gửi lệnh Unlike -> Back-end chửi "Chưa like mà"
      else if (errorMsg.includes('not liked') || errorMsg.includes('not found')) {
        toast.info("Đã đồng bộ: Bài hát này chưa được Yêu thích!");
        setLocalLikeSync(false); // Ép trái tim rỗng!
      } 
      // Lỗi mạng hoặc lỗi server thật
      else {
        toast.error("Không thể thao tác Yêu thích lúc này!");
      }
    } finally {
    ...
    <footer className="h-[90px] w-full bg-[#181818] border-t border-[#282828] flex items-center justify-between px-4 z-50 flex-shrink-0 select-none">
      ...
      <div className="flex flex-col items-center justify-center w-[40%] max-w-[722px] gap-2">
        <div className="flex items-center gap-6">
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill={currentlyLiked ? "#1ed760" : "none"} stroke={currentlyLiked ? "#1ed760" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="transition-colors duration-200">
          ...
          <button
            ...
            className={`transition-colors duration-200 ${!currentSong ? 'opacity-50 cursor-not-allowed text-[#b3b3b3]' : isReposted ? 'text-[#1ed760] hover:text-[#1ed760]/80' : 'text-[#b3b3b3] hover:text-white'}`}
